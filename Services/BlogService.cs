...
public class BlogService : IBlogService
{
  ...
  {
    var blogPost = await _context.BlogPosts.Include(c => c.Tags).FirstOrDefaultAsync(c => c.Id == blogPostId);
  ...
  public Task<IEnumerable<BlogPost>> GetRecentBlogPostsAsync() => throw new NotImplementedException();

  public Task<IEnumerable<BlogPost>> GetRecentBlogPostsAsync(int? count) => throw new NotImplementedException();

  public Task<bool> IsTagOnBlogPostAsync(int? tagId, int? blogPostId) => throw new NotImplementedException();

  public async Task RemoveAllBlogPostTagsAsync(int blogPostId)
  {
    var blogPost = await _context.BlogPosts.Include(b => b.Tags).FirstOrDefaultAsync(c => c.Id == blogPostId);
  ...
  public async Task<List<Tag>> GetTagsAsync() => await _context.Tags.ToListAsync();
  ...
  public async Task<IEnumerable<BlogPost>> GetFavoriteBlogPostsAsync(string? blogUserId)
  {
    ...
    if (!string.IsNullOrEmpty(blogUserId))
    {
      ...
      if (blogUser != null)
        blogPosts = await _context.BlogPosts.Where(b => b.Likes.Any(l => l.BlogUserId == blogUserId && l.IsLiked == true) && b.IsPublished == true && b.IsDeleted == false)
                                            .Include(b => b.Likes)                                                                                                                                      
                                            .Include(b => b.Comments)                                                                                                                                   
                                            .Include(b => b.Category)                                                                                                                                   
                                            .Include(b => b.Tags)                                                                                                                                
